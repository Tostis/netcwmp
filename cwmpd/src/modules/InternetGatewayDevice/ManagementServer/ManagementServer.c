...
int cpe_get_igd_ms_connectionrequesturl(cwmp_t * cwmp, const char * name, char ** value, pool_t * pool)
{
    ...
    strcpy(local_ip, "here.goes.my.public.ip"); // TODO so I go hardcoded
    int port = cwmp_conf_get_int("cwmpd:httpd_port");
    snprintf(buf, 256, "http://%s:%d/thecontext", local_ip, port);
    *value = PSTRDUP(buf);
    return FAULT_CODE_OK;
}
...
//InternetGatewayDevice.ManagementServer.ParameterKey //TODO salvare in configurazione o basta che sia settato in pool?
int cpe_get_igd_ms_parameterkey(cwmp_t * cwmp, const char * name, char ** value, pool_t * pool)
{
    *value = cwmp_conf_pool_get(pool, "cwmp:parameterkey");
    return FAULT_CODE_OK;
}
...
{
	int ret = cwmp_conf_set("cwmp:parameterkey", value);
	if (ret == 1)
    	return FAULT_CODE_OK;
	return FAULT_CODE_9002;
}
